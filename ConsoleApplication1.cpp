...
double S0(const int N)
{
	double s = 0;
	for (int i = 1; i <= N; i++)
		s += (1 + i * static_cast<double>(1) / N) / pow(i, 2);
	return s;
}
double S1(const int N, const int i)
{
	if (i > N)
		...
		return (1 + i * static_cast<double>(1) / N) / pow(i, 2) + S1(N, i + 1);
}
double S2(const int N, const int i)
{
	if (i < 1)
		...
		return (1 + i * static_cast<double>(1) / N) / pow(i, 2) + S2(N, i - 1);
}
double S3(const int N, const int i, double t)
{
	t = t + (1 + i * static_cast<double>(1) / N) / pow(i, 2);
	if (i >= N)
		return t;
	else
		return S3(N, i + 1, t);
